...
xlabel('X'); ylabel('Y'); title('Scenario: Target (Blue) vs Interferers (Magenta)');
view(2); xlim([-50 60]); ylim([-40 40]); legend([hUE1, hUE2, hUE3]);
...
for t = 1:nSymbols
    ...
    if abs(rad2deg(az_geo3)) > 90, elem_gain3 = 1e-4; else, elem_gain3 = cos(az_geo3); end
    for p = 1:nPaths
        ...
        sv = exp(1j * 2*pi/Pars.lambda * (y_pos*ky + z_pos*kz));
        del = exp(-1j*2*pi*(0:Nfft-1).'*path_delays_samples(p)/Nfft).';
        H3 = H3 + (path_gains(p) * elem_gain3 * exp(1j*accum_phase3(p)) * sv) * del;
    end
    H3 = H3 * sqrt(pl3);
    ...
    for k = 1:Nfft
        RX_Total(:,k) = H1(:,k)*tx1_vec(k) + H2(:,k)*tx2_vec(k) + H3(:,k)*tx3_vec(k) + noise(:,k);
    end
    ...
    for idx = 1:length(dataIndices)
        ...
        P_I = (abs(w_k' * H2(:,k))^2 + abs(w_k' * H3(:,k))^2) * TxPower_per_SC; 
