...
ax2 = nexttile([1 1]);
hConst = plot(NaN, NaN, '.', 'Color', [1 1 0], 'MarkerSize', 4);
grid(ax2, 'on'); axis(ax2, 'square'); xlim([-2 2]); ylim([-2 2]);
title('RX Constellation (Serving BS)');
xline(0); yline(0);

ax2b = nexttile([1 1]);
hConst2 = plot(NaN, NaN, '.', 'Color', [1 1 1], 'MarkerSize', 4);
...
ax4 = nexttile([1 1]);
hSINR1 = plot(NaN, NaN, 'LineWidth', 1.5, 'Color', 'r', 'DisplayName','BS1 (SINR pre-BF)'); hold on;
hSINR2 = plot(NaN, NaN, 'LineWidth', 1.5, 'Color', 'g', 'DisplayName','BS2 (SINR pre-BF)');
hSINR_post = plot(NaN, NaN, 'LineWidth', 2, 'Color', 'c', 'DisplayName','Serving (post-BF)');
grid(ax4, 'on'); title('SINR: Pre-BF (Handover Decision) vs Post-BF (Actual)');
xlabel('Symbol'); ylabel('dB'); ylim([-50 60]); legend('Location','best');

ax5 = nexttile([1 1]);
hEVM1 = plot(NaN, NaN, 'LineWidth', 1.5, 'Color', 'r', 'DisplayName','Serving BS');
grid(ax5, 'on'); title('EVM (Serving BS)');
...
mse_hist2 = []; sinr_hist2 = []; const_buf2 = []; 
sinr_post_hist = [];   % SINR post-beamforming (serving BS)
...
for t = 1:nSymbols
    ...
    % ============================================================
    % SINR CALCULATION FOR HANDOVER DECISION
    % ============================================================
    % Per la decisione di handover usiamo una metrica INDIPENDENTE dal beamformer:
    % Calcoliamo la potenza del canale (simile a RSRP) basata sulla 
    % channel matrix H, NON sui pesi W che potrebbero non essere aggiornati.
    %
    % RSRP-like metric: ||H_UE1||^2 (potenza media del canale verso UE1)
    % Questo è quello che l'UE misurerebbe dai Reference Signals
    
    % Potenza canale UE1 verso BS1 (media su tutte le sottoportanti e antenne)
    channel_power_bs1 = mean(abs(H1_BS1(:)).^2);
    
    % Potenza canale UE1 verso BS2
    channel_power_bs2 = mean(abs(H1_BS2(:)).^2);
    
    % Potenza interferenza + rumore (per calcolo SINR realistico)
    % Interferenza da UE2 e UE3 come vista dalla BS
    interf_power_bs1 = mean(abs(H2_BS1(:)).^2) + mean(abs(H3_BS1(:)).^2);
    interf_power_bs2 = mean(abs(H2_BS2(:)).^2) + mean(abs(H3_BS2(:)).^2);
    
    % SINR pre-beamforming (metrica per handover)
    % Questo rappresenta la qualità del link PRIMA dell'elaborazione
    sinr_pre_bf_bs1 = 10*log10(max(channel_power_bs1, 1e-12) / max(interf_power_bs1 + noise_power_per_sc, 0));
    sinr_pre_bf_bs2 = 10*log10(max(channel_power_bs2, 1e-12) / max(interf_power_bs2 + noise_power_per_sc, 0));
    
    % ============================================================
    % SINR POST-BEAMFORMING (per valutare performance effettiva)
    % ============================================================
    % Questo è il SINR dopo che il beamformer ha elaborato il segnale
    % Valido solo per la BS che sta facendo beamforming attivo
    
    if UE1.connected_bs == 1
        % BS1 sta facendo beamforming attivo
        ...
        Pi1 = mean(abs(int1(pilotIndices)).^2) + mean(abs(noi1).^2);
        sinr_post_bf = 10*log10(max(Ps1,1e-12)/max(Pi1,1e-12));
    else
        % BS2 sta facendo beamforming attivo
        ...
        Pi2 = mean(abs(int2(pilotIndices)).^2) + mean(abs(noi2).^2);
        sinr_post_bf = 10*log10(max(Ps2,1e-12)/max(Pi2,1e-12));
    end
    
    % Salva metriche per plotting
    % sinr_hist/sinr_hist2 = SINR pre-beamforming (per confronto handover)
    sinr_db1 = sinr_pre_bf_bs1;
    sinr_db2 = sinr_pre_bf_bs2;
    sinr_hist(end+1) = sinr_db1;
    sinr_hist2(end+1) = sinr_db2;
    sinr_post_hist(end+1) = sinr_post_bf;  % SINR post-beamforming della serving BS
    ...
    % EVM on data subcarriers - SOLO per la serving BS
    if UE1.connected_bs == 1
        pilot_ref = pilotVal * Tx_Amplitude;
        pilot_out = y_out1(pilotIndices).';
        norm_factor = mean(pilot_out ./ pilot_ref);
        if abs(norm_factor) < 1e-9, norm_factor = 1; end
        ref_sym = TX_Grid1(dataIndices, t);
        y_data = y_out1(dataIndices).' / norm_factor;
    else
        pilot_ref = pilotVal * Tx_Amplitude;
        pilot_out = y_out2(pilotIndices).';
        norm_factor = mean(pilot_out ./ pilot_ref);
        if abs(norm_factor) < 1e-9, norm_factor = 1; end
        ref_sym = TX_Grid1(dataIndices, t);
        y_data = y_out2(dataIndices).' / norm_factor;
    end
    err_evm = y_data - ref_sym;
    evm_curr = sqrt(mean(abs(err_evm).^2) / max(mean(abs(ref_sym).^2),1e-12));
    ...
    if mod(t, 20) == 0
        ...
        set(hSINR_post,'XData', 1:length(sinr_post_hist), 'YData', sinr_post_hist);
        ...
        % Determina SINR corrente per display
        if UE1.connected_bs == 1
        ...
        set(hText, 'String', sprintf(['Sym: %d\n' ...
            '-------------------\n' ...
            'Connected to: BS%d\n' ...
            'Pre-BF SINR (serving): %.1f dB\n' ...
            'Pre-BF SINR (neighbor): %.1f dB\n' ...
            'Post-BF SINR: %.1f dB\n' ...
            'EVM: %.3f\n' ...
            '-------------------\n' ...
            'HO Status: %s\n' ...
            'Total Handovers: %d\n' ...
            'Hysteresis: %.1f dB'], ...
            t, UE1.connected_bs, sinr_serving, sinr_neighbor, sinr_post_bf, evm_curr, ...
