...
% --- 3. Combinato (Colonna destra - occupa 3 righe) ---
subplot(3,3,[5 6 8 9]);
...
% --- 6. POWER (In basso a sinistra) - AGGIUNTO ---
subplot(3,3,7);
...
for currentFrame = 1:Pars.numFrame
    ...
    if UE(1).DOA_variation > 1  % MODIFICATO: era > 2 || mod(currentFrame, 20)
        ...
        [N,L] = size(rx_signal);
        for k=1:maxUsers
            UE(k).weights = zeros(L,1);  % MODIFICATO: reset pesi
...
sData.hPow_line1 = hPow_line1; sData.hPow_line2 = hPow_line2; % AGGIUNTO
% Handles Surfaces
sData.hSurf1 = hSurf1;
sData.hSurf2 = hSurf2;
